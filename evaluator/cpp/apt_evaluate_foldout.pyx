...
def apt_evaluate_foldout(ranking_scores, ground_truth, top_k = 20, thread_num=None):
    metrics_num = 5
    groups_num, rank_len = np.shape(ranking_scores)
    if groups_num != len(ground_truth):
    ...
    # store ranks results
    top_rankings = np.zeros([groups_num, top_k], dtype=int_type)
    cdef int *rankings_pt = <int *>np.PyArray_DATA(top_rankings)

    # get top k rating index
    c_top_k_array_index(scores_pt, rank_len, groups_num, top_k, thread_num, rankings_pt)

    # the pointer of ground truth, the pointer of the length array of ground truth
    ground_truth_pt = <int **> PyMem_Malloc(groups_num * sizeof(int *))
    ground_truth_num = np.zeros([groups_num], dtype=int_type)
    ground_truth_num_pt = <int *>np.PyArray_DATA(ground_truth_num)
    for u in range(groups_num):
    ...
    #evaluate results
    results = np.zeros([groups_num, metrics_num*top_k], dtype=float_type)
    results_pt = <float *>np.PyArray_DATA(results)

    #evaluate
    evaluate_foldout(groups_num, rankings_pt, top_k, ground_truth_pt, ground_truth_num_pt, thread_num, results_pt)
